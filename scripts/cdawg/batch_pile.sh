...
for split in {$START..$END}; do
    ...
    cmd="screen -dmS cdawg bash -c 'export SPLIT=$split && chmod +x rusty-dawg-startup/wrap_startup.sh && rusty-dawg-startup/wrap_startup.sh && sudo shutdown -h now; exec bash'"
