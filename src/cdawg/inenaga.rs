...
//
// Similar implementation in Python: https://github.com/raedwulf/PyCDAWG/blob/master/cdawg.py
...
impl Cdawg {
    ...
    fn split_edge(&mut self, state: NodeIndex, start: usize, end: usize) -> NodeIndex {
        ...
        let mut_ref = self.graph.get_edge_mut(edge_idx.unwrap());
        mut_ref.set_weight(CdawgEdgeWeight::new(token, found_start, found_start + end - start));
        ...
        let new_weight = CdawgEdgeWeight::new(split_token, found_start + end - start + 1, found_end);
